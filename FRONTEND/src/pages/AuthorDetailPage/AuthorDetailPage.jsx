...
export default function AuthorDetailPage({ authorId }) {
  ...
  if (notFound) {
    return <><SharedNav /><main className="author-page"><div className="author-empty"><div className="eyebrow">THE PAGE LEFT THE ARCHIVE</div><h1>Author not found.</h1><p>{error}</p><Link className="button ghost" to="/explore">Back to explore</Link></div></main></>
  }

  return <><SharedNav /><main className="author-page"><div className="author-head"><div className="author-avatar">{author?.profileImage ? <img src={author.profileImage} alt="" /> : (author?.name || 'A').slice(0, 1)}</div><div><div className="eyebrow">{author?.authorType === 'USER' ? 'USER AUTHOR' : 'AUTHOR'} · {books.length} BOOK{books.length === 1 ? '' : 'S'}</div><h1>{author?.name || 'Loading author…'}</h1>{author?.user ? <Link className="author-handle" to={`/users/${author.username}`}>@{author.username}</Link> : null}{author?.placeOfBirth && <p className="author-place"><FiCalendar /> {author.placeOfBirth}</p>}</div></div>{author?.biography && <p className="author-bio">{author.biography}</p>}<section className="author-books"><div className="eyebrow">THEIR WORKS</div><h2>Book collection</h2>{books.length ? <div className="author-book-grid">{books.map(book => <Link className="author-book-card" to={`/books/${book.id}`} key={book.id}>{book.coverImage || book.thumbnailUrl ? <img src={book.coverImage || book.thumbnailUrl} alt="" /> : <span className="author-book-fallback">SV</span>}<h3>{book.title}</h3><p>{book.genre || 'Story'} · {book.publicationDate ? String(book.publicationDate).slice(0, 4) : '—'}</p></Link>)}</div> : <div className="author-empty"><div><FiBookOpen /></div><h3>No published works yet.</h3><p>This author hasn't published any stories yet.</p></div>}</section></main></>
}
