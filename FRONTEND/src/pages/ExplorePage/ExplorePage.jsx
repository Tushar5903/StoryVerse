...
function BookCard({ book, count, index }) {
  ...
  return <motion.div className="discover-motion" role="listitem" key={book.id} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: index * .05 }}>
...
function Section({ title, subtitle, state, view }) {
  return <section className="discover-section" aria-label={title}>
    <div className="discover-head-row">
      <div className="discover-heading"><h2>{title}</h2><p>{subtitle}</p></div>
      <Link className="discover-viewall" to={`/search?view=${view}`}>View All <FiArrowRight /></Link>
    </div>
    {state.loading ? <div className="discover-empty">Curating the shelves…</div> : state.books.length
      ? <Row label={title}>{state.books.map(({ book, count }, index) => <BookCard key={book.id} book={book} count={count} index={index} />)}</Row>
...
function SearchResults({ query }) {
  ...
  return <div className="discover-page"><header className="discover-head"><Link className="discover-back" to="/explore"><FiArrowLeft /> Back to Explore</Link><div className="eyebrow">SEARCH</div><h1>Results for “{query}”</h1></header>{state.loading ? <div className="discover-empty">Searching the shelves…</div> : state.books.length ? <div className="discover-grid">{state.books.map((book, index) => <BookCard key={book.id} book={book} index={index} />)}</div> : <div className="discover-empty">No books match “{query}”. Try another word.</div>}</div>
}
...
function ExploreContent({ initialGenre = '' }) {
  ...
  return <div className="discover-page"><header className="discover-head"><div className="eyebrow">THE DIGITAL LITERARY ARCHIVE</div><h1>Explore.</h1>{initialGenre ? <Link className="discover-genre-chip" to="/explore" title="Clear genre filter">{initialGenre}<span aria-hidden="true">×</span></Link> : null}</header><Section title="Talk Of The Town" subtitle="The latest books everyone is discovering." state={sections.talk} view="talk" /><Section title="User's Books" subtitle="Fresh stories shared by our community." state={sections.users} view="users" /><Section title="Books For Review" subtitle="Read, review, and help others discover great books." state={sections.review} view="review" /><Section title="Most Famous Books" subtitle="The books loved most by our readers." state={sections.famous} view="famous" /></div>
}
