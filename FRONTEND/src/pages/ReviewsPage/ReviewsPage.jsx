...
export default function ReviewsPage() {
  ...
  return <main className="reviews-page"><aside className="review-profile-card"><div className="review-profile-avatar">{user?.profileImage ? <img src={user.profileImage} alt="" /> : (user?.name || 'U').slice(0, 1)}</div><h2>{user?.name || 'StoryVerse reader'}</h2><strong>@{user?.username || 'reader'}</strong><div className="review-profile-stats"><span><b>{reviews.length}</b>Reviews Posted</span><span><b>{collection.length}</b>Collections</span></div><p>{user?.bio || 'I love stories, books, and thoughtful criticism.'}</p>{socialLinks(user).length > 0 && <div className="review-socials">{socialLinks(user).map(link => { const Icon = link.icon; return <a key={link.key} href={link.href} target="_blank" rel="noopener noreferrer" title={link.label} aria-label={link.label}><Icon size={15} /></a> })}</div>}<div className="review-follow"><FiCalendar /> Joined recently</div><Link className="button ghost" to="/settings">Edit Profile</Link></aside><section className="review-feed"><div className="review-mode"><button className={mode === 'reviews' ? 'active' : ''} onClick={() => setMode('reviews')}><FiStar /> Reviews</button><button className={mode === 'collection' ? 'active' : ''} onClick={() => setMode('collection')}><FiBookOpen /> Collections</button></div>{mode === 'reviews' ? <>
