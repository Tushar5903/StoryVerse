...
import { scoreOf, countsOf } from '../../utils/verdicts'
    ...
    <div className="lb-item">
      ...
      <Link className="lb-poster" to={`/books/${item.book.id}`} aria-label={`Open ${item.book.title}`}>
        {item.book.coverImage || item.book.thumbnailUrl
          ? <img src={item.book.coverImage || item.book.thumbnailUrl} alt={item.book.title} loading="lazy" />
          : <span>SV</span>}
      </Link>
...
export default function LeaderboardPage() {
  ...
  const fetchRows = useCallback(() => listBooks('?size=100')
  ...
  const ranked = useMemo(() => {
    ...
    const verdictPool = filter === 'ALL' ? withStats : withStats.filter(item => item.reviews.some(review => review.verdict === filter))
  ...
  return <><SharedNav /><main className="leaderboard-page"><div className="lb-desktop"><aside><div className="eyebrow">FILTER BY</div>{filters.map(([key, label, color]) => <button className={`verdict-filter ${filter === key ? 'active' : ''}`} onClick={() => setFilter(key)} key={key}><i className={color} />{label}</button>)}<div className="eyebrow sort-label">CONTENT TYPE</div>{TYPE_TABS.map(([key, label]) => <button className={`verdict-filter${type === key ? ' active' : ''}`} onClick={() => setType(key)} key={key}>{label}</button>)}<div className="eyebrow sort-label">SORT BY</div><select className="leader-sort" value={sort} onChange={event => setSort(event.target.value)}><option value="votes">Most reviewed</option><option value="score">Highest rated</option></select></aside><section><div className="eyebrow">THE PEOPLE'S VERDICT</div><h1>Top 10 books.</h1>{error ? <div className="leader-error">{error}</div> : loading ? <div className="leader-error">Counting the votes…</div> : <div className="ranked-list">{ranked.map((item, index) => <div className="ranked-row" key={item.book.id}><strong>{String(index + 1).padStart(2, '0')}</strong><div className="rank-cover">{item.book.coverImage || item.book.thumbnailUrl ? <img src={item.book.coverImage || item.book.thumbnailUrl} alt="" /> : <span>SV</span>}</div><div className="rank-copy"><Link to={`/books/${item.book.id}`}><h2>{item.book.title}</h2></Link><p>{item.book.authorName || 'Unknown author'} · {item.votes} review{item.votes === 1 ? '' : 's'}</p></div><Meter reviews={item.reviews} compact /></div>)}</div>}{!error && !loading && !ranked.length && <div className="leader-error">No ranking records found for this filter yet. Be the first to leave a verdict.</div>}</section></div><div className="lb-mobile"><div className="eyebrow">THE PEOPLE'S VERDICT</div><h1>Top 10 books.</h1><div className="lb-filter-bar"><div className="lb-pill-strip" role="group" aria-label="Filter by verdict">{filters.map(([key, label, color]) => <button type="button" className={`lb-pill${filter === key ? ' active' : ''}`} aria-pressed={filter === key} onClick={() => setFilter(key)} key={key}><i className={color} />{label}</button>)}</div><SortDropdown variant="panel" prefix="" value={sort} onChange={setSort} options={SORTS} /></div><div className="lb-tabs" role="tablist" aria-label="Content type">{TYPE_TABS.map(([key, label]) => <button type="button" role="tab" className={`lb-tab${type === key ? ' active' : ''}`} aria-selected={type === key} onClick={() => setType(key)} key={key}>{label}</button>)}</div>{error ? <div className="lb-state"><p>Unable to load leaderboard.</p><button type="button" className="lb-retry" onClick={retry}>Try Again</button></div> : loading ? <div className="lb-list">{[...Array(5)].map((_, index) => <SkeletonRow key={index} />)}</div> : ranked.length ? <div className="lb-list">{ranked.map((item, index) => <LeaderboardRow item={item} index={index} key={item.book.id} />)}</div> : <div className="lb-state"><p>No rankings available for this filter yet. Be the first to leave a verdict.</p></div>}</div></main><Footer /></>
}
